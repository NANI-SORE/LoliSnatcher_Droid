...
class BooruEditFormController {
  BooruEditFormController(Booru initialBooru, {required bool trustInitialConnection}) {
    ...
    authLogin.text = initialBooru.authLogin ?? '';
    authPassword.text = initialBooru.authPassword ?? '';
  ...
  final authLogin = TextEditingController();
  ...
  Booru toBooru({BooruType? type, String? tags}) {
    return Booru.withKey(
      ...
      authLogin: authLogin.text.isEmpty ? null : authLogin.text,
      authPassword: authPassword.text.isEmpty ? null : authPassword.text,
    );
  }
